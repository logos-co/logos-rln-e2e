...
proc lipOptions(rate: int, optionsJson: string): Result[string, string] =
  ...
  ## harness passes (the delegated set, or nothing at all) flattens into it;
  ## since wire 0.6.0 the array IS the module wire and the bridge passes it
  ## through verbatim. A direct registration needs no options beyond the rate:
  ## the registry charges the module's own payer, in native.
  var arr = newJArray()
...
proc rlnconsumerRegisterMembership*(
  ...
  ## optionsJson: "" or "{}" (direct — nothing names who pays since module
  ## wire 0.8.0) or {"delegated":"true","gifter_peer_id":...,
  ## "gifter_multiaddr":...} (gifter) — flattened into the seam's LIP
  ## RegistryOptions key/value array.

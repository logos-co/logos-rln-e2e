...
#
#   open wallet -> sync -> fund the node's own payer (direct)
...
# only the staged wallet_config.json, so the module derives a payer nothing
# else holds. Before daemon_start — the home rides the daemon's command line.
...
# ---------- wallet + funding (direct module calls: harness domain) -----------
section "wallet"
...
# Single-asset registry: the module's own payer signs and pays the price and
# the fee from its NATIVE balance. Delivery never owned this step either —
# the transfer is the harness's, exactly as the claim used to be.
...
# No options beyond the rate limit the consumer folds in itself: since the
# registry became single-asset nothing in the options names who pays.
OPTIONS_JSON="{}"

...
#
#   n1 (gifter): RLN stack + libp2p_module + rln_gifter_module, its own payer
#      funded with NATIVE balance, `rln_gifter_module.serve` with NO
#      authVerifiers (open gifter). serve's `wallet` is that payer: the
#      registry is single-asset, so the account the gifter names signs the
#      Register tx, pays the price and pays the fee. The gifter needs funds
#      but NO membership of its own.
#   n2 (client): RLN stack + libp2p + gifter(client half) + nim_rln_consumer,
#      NO funds — its own wallet, derived from nothing and never funded, used
...
for _v in LOGOSCORE E2E_MODULES_DIR E2E_SEQUENCER E2E_WALLET_HOME E2E_CONFIG_ACCOUNT \
          E2E_TREE_ID E2E_PAYER E2E_CONFIRM_TIMEOUT_S E2E_POLL_INTERVAL_S \
...
# Both nodes get a wallet of their own (a home carrying only the staged
# wallet_config.json, so each module derives a payer nothing else holds).
# That is what makes "the gifter pays, the client has nothing" a fact rather
# than a wording: handing both the deployment's shared payer would fund the
# client too, and the scenario could no longer tell who paid.
...
# The gifter is the one node here that must hold money: it pays for somebody
# else's membership out of its own NATIVE balance. n2 is deliberately left
# alone — the whole point is that a fundless consumer still becomes a prover.
GPAYER=$(wallet_payer n1) || die "the gifter's registry module published no payer"
wallet_fund n1 >/dev/null || die "funding the gifter's payer $GPAYER never landed"
say "gifter payer funded: $GPAYER"
...
# the account `wallet` names (the gifter node's own funded payer; the gifter
# forwards it as register_member's payer). NOTE: a Rust module's string param
# takes a bare JSON object from the CLI (the register scenario's options
# precedent); an @argfile object arrives brace-stripped.
...
# The client's own wallet home: chain reads only, never funded, never shared
# with n1's mutable storage.json (truncate-in-place writes). It carries no
# storage of its own either, so its derived payer is a different account from
# the gifter's — a copied seed would derive the SAME one.
...
# "Fundless" has to be a fact, not a wording: if the client derived the same
# account the gifter was funded on, a self-funded registration would be
# indistinguishable from a delegated one.
CPAYER=$(wallet_payer n2) || die "the client's registry module published no payer"
[ "$CPAYER" != "$GPAYER" ] \
    || die "the client derived the gifter's funded payer ($GPAYER) — this run could not tell a delegated registration from a self-funded one"
say "client payer: $CPAYER (never funded; the gifter's is $GPAYER)"
...
echo "e2e:   leaf_index $LEAF, gifter $GPEER paid from $GPAYER"

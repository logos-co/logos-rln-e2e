...
# exactly the ones no other scenario exercises. After one payer-funded
...
# n1 pays for its own membership out of its own wallet (a home carrying only
# the staged wallet_config.json, so the module derives a payer nothing else
# holds). Before daemon_start: the home rides the daemon's command line. n2,
# which only probes custody and never registers, keeps the staged home.
...
# The registry is single-asset: the module's own payer signs the Register tx
# and pays both the price and the fee from its NATIVE balance. Nothing mints
# native balance, so the one transfer the module stack cannot do for itself
# happens here, before the registration below.

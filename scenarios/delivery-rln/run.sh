...
# integration against the REAL RLN module stack. It runs on UPSTREAM
# logos-delivery and logos-delivery-module — no fork of either (last verified
# 2026-09-15 from fresh clones of both master tips: logos-delivery-module
# af1f6ac, logos-messaging/logos-delivery 4856c812).
#
...
#      Nothing here needs logos-delivery's fork; the rlnStartRequest,
...
answer_event() {
    ...
    rlnRegisterRequest)
        ...
        # a re-register short-circuit, never a double mint — and on a node the
        # module already provisioned, it short-circuits to that membership.
        local reg note
...
# would give every node the same seed and therefore the SAME derived account —
# "its own payer" would be a fiction, and they would race one nonce.
#
# It has to happen here rather than in the wallet section below because the
# home reaches the module as an env var on the daemon's command line.
for n in $NODES_ALL; do
...
# permanently cold root window. And since the library's bring-up gate needs a
# membership on every rln-enabled node, each node derives and funds ITS OWN
# holding out of its own wallet copy (made above, before either daemon
# started), so n1's holding key is not in n2's wallet — the sequencer rejects
# a spend from an account the signer does not hold.
section "wallets"
CHAIN_HEAD=$(chain_head) || die "cannot probe chain head at $E2E_SEQUENCER"
say "syncing wallets to chain head $CHAIN_HEAD"
for n in $NODES_ALL; do
    say "$n: wallet home $(node_wallet_home "$n")"
    wallet_open "$n" || die "$n: wallet open failed"
...
# an active/grace_period membership on every rln-enabled node, and the module
# stack acquires one by itself: `start` above named the registry, and its
# provisioning task waits for the wallet, waits until that wallet can afford a
# registration, and registers a registry-wide membership.
#
# So the ONLY thing this scenario did for the membership was fund the payer.
# That is the whole point: the previous shape had the test derive a holding,
# claim tokens and call register_membership — which meant the e2e was the thing
# making the system work, and the path a real deployment takes had never run.
#
# What is asserted is therefore an OUTCOME, not a sequence: within the
# confirmation budget, each node reaches active on its own.
section "provisioning (the module registers itself; the harness only funded it)"
await_provisioned() {
    local node="$1" state_json state
    ...
    for _t in $(seq 1 "$(polls "$E2E_CONFIRM_TIMEOUT_S" "$E2E_POLL_INTERVAL_S")"); do
        ...
        # `unknown` carries a provisioning object while the task is working,
        # so a slow run says which step it is on rather than looking stuck.
        case "$state" in
        ...
        case "$state" in
            ...
            failed) die "$node: provisioned registration FAILED on chain: $state_json" ;;
        esac
        sleep "$E2E_POLL_INTERVAL_S"
    done
    [ "$state" = "active" ] || [ "$state" = "grace_period" ] \
        || die "$node: the module never provisioned a membership (last state: ${state:-<none>}). \
...
await_provisioned n1
await_provisioned n2
LEAF=$(gv LEAF n1)
...
while [ "$ATTEMPT" -lt "$SEND_ATTEMPTS" ]; do
    ...
    PROP=$(node_wait_event n1 delivery_module messagePropagated "$EVT_TIMEOUT" "$REQID") || {
        node_wait_event n1 delivery_module messageError 1 "$REQID" >/dev/null \
            && die "messageError for requestId $REQID (gate or generate leg failed — library: $(node_logs n1 | grep -m1 'usable RLN membership\|Failed to verify RLN membership\|Failed to attach RLN proof' || echo 'see n1 daemon log'))"

...
| `delivery-fleet-rln` | **active** | three containerised relays, each holding its own membership and validating what it forwards, with two host endpoints that meet only at the fleet and never dial each other. Sends at a steady rate for `E2E_FLEET_DURATION_S` across several RLN epochs; asserts every send was proved, delivery stayed above a floor, and the budget refilled. Needs the relay image |
